...
describe('processExpenses', function() {
  describe('process', function() {
    it('Contains validation errors', async function() {
      ...
      expect(result.errorLines.length).to.eql(3);
      ...
      expect(result.errorLines[1].line).to.eql('2021-04,29,194.32,Groceries,Metro');
      expect(result.errorLines[1].description).to.eql('Date format must be YYYY-MM-DD.');

      expect(result.errorLines[2].line).to.eql('2021-04-30,45.87,Electronics');
      expect(result.errorLines[2].description).to.eql('Expected 4 fields, got 3.');
